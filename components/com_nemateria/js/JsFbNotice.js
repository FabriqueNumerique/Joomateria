/*1322006683,177604148,JIT Construction: v476372,en_US*/
...
if(!FB.dynData) {
    FB.dynData = {
        ...
        "resources":{
            ...
            "api_channel":1321852851,
            "api_server":1321852851,
            "www_channel":1321852784,
            "xd_proxy":1321852714,
            "xd_comm_swf_url":"http:\/\/connect.facebook.net\/rsrc.php\/v1\/yK\/r\/RIxWozDt5Qq.swf",
...
if(!window.FB.Share){
    FB.Share={
        ...
        addQS:function(d,c){
            var a=[];
            for(var b in c)if(c[b])a.push(b.toString()+'='+encodeURIComponent(c[b]));return d+'?'+a.join('&');
        },
        getUrl:function(a){
        ...
        displayBox:function(a,d){
            if(typeof(a.fb_count)=='number'&&a.fb_count>=d)for(var c=1;c<=2;c++){
                ...
                b.className=b.className.replace('fb_partager_no_count','');
    ...
    renderButton:function(c){
        ...
        var i=c.innerHTML.length>0?c.innerHTML:'Partager';
        var g={
            u:k,
        ...
        if(h.indexOf('count')>=0){
            var e=(h=='box_count');
            var f=(e?'top':'right');
            var d='<span class=\'fb_partager_size_'+j+' '+(e?'fb_partager_count_wrapper':'')+'\'>';
            var b='<span class=\'fb_partager_count_nub_'+f+' fb_partager_no_count\'></span>';
            b+='<span class=\'fb_partager_count fb_partager_no_count'+' fb_partager_count_'+f+'\'>'+'<span class=\'fb_partager_count_inner\'>&nbsp;</span></span>';
    ...
    renderAll:function(d){
        var c=document.getElementsByName('fb_partager');
        var a=c.length;
        for(var b=0;b<a;b++){
    ...
    fetchData:function(){
        ...
        c.src=this.addQS((FB.isSecure()?'https:':'http:')+'//api.facebook.com/restserver.php',{
            ...
            urls:'['+a.join(',')+']',
            format:'json',
            callback:'fb_partagerpro_render'
        });
        this.resetUrls();
        this.insert(c);
    },
    ...
    _onFirst:function(){
        ...
        this.insert(b);
        this.resetUrls();
        window.fb_partagerpro_render=function(c){
