(function(a,b){
    function O(a){
        for(var b=0;b<J.bundles.length;b++)for(var c in J.bundles[b])if(c!=a&&v(J.bundles[b][c],a)>-1)return c
            ...
            function N(){
        a.using=m,a.provide=n,a.define=o,a.loadrunner=p;
        return M
        }
        function L(a){
        var b,c;
        ...
        typeof a[0]=="string"&&(c=a.shift()),u(a[0])&&(b=a.shift()),d=a.shift();
        ...
        function F(){
        var a=t(arguments),b,c;
        typeof a[0]=="string"&&(b=a.shift()),c=a.shift();
        return E(b,c)
        }
        function E(a,b){
        var d;
        !a&&c&&(d=k||D()),d?(delete l[d.scriptId],d.body=b,d.execute()):(j=d=new A(a,b),i[d.id]=d);
        return d
        }
        function D(){
    ...
    function v(a,b){
    for(var c=0,d;d=a[c];c++)if(b==d)return c;return-1
    }
    function t(a){
    return Array.prototype.slice.call(a)
    }
    function s(a){
    for(var b=1,c;c=arguments[b];b++)for(var d in c)a[d]=c[d];return a
    }
...
y.prototype.then=function(b){
    var c=this;
    this.started||(this.started=!0,this.start()),this.completed?b.apply(a,this.results):(this.callbacks=this.callbacks||[],this.callbacks.push(b));
    return this
    ...
    var a=this,b=0,c=[];
    a.collectResults&&(c[0]={}),function d(){
        var e=a.deps[b++];
        e?e.then(function(b){
...
__twttrlr(function(using, provide, loadrunner, define) {
    provide("util/iframe",function(a){
        ...
        a(function(a){
            ...
            var c=b.firstChild.cloneNode(!1);
            c.src=a.url,c.className=a.className||"";
            for(var d in a.css)c.style[d]=a.css[d];c.title=a.title||"";
            if(a.replace){
                var e=a.replace.parentNode;
                e.replaceChild(c,a.replace)
    ...
    provide("util/querystring",function(a){
        function f(a,b){
            ...
            function d(a){
            var c=[];
            for(var d in a)a[d]!==null&&typeof a[d]!="undefined"&&c.push(b(d)+"="+b(a[d]));return c.sort().join("&")
            }
            function c(a){
    ...
    provide("tfw/widget/base",function(a){
        using("util/util","util/domready","util/nodeselect","util/querystring","util/iframe",function(b,c,d,e,f){
            function o(a){
                ...
                for(var e in a){
                    ...
                    for(var f=0,g;g=b[f];f++)c=new a[e](g),j.list.push(c),j.byId[c.id]=c,c.render(i)
                        }
                    }
                var g=0,h,i,j={
            list:[],
        ...
        b.aug(k.prototype,{
            _:function(a){
                var b=this.lang;
                if(!b||!this.langs.hasOwnProperty(b))b="en";
                return this.langs[b][a]
                },
            add:function(a){
            ...
            generateId:function(){
                return this.originElement.id?this.originElement.id:"twitter_tweet_button_"+g++
                }
            }),a({
        Base:k,
        init:m,
        embed:n,
        find:o,
        TWITTER_PROFILE_URL:/^https?\:\/\/(?:www\.)?twitter\.com\/(?:#!?\/)?([a-zA-Z0-9_]{1,20})\/?$/
    })
    })
});
provide("util/events",function(a){
    using("util/util",function(b){
        function d(){
            ...
            var c={
            bind:function(a,b){
                this._handlers=this._handlers||{},this._handlers[a]=this._handlers[a]||[];
                return this._handlers[a].push(b)
                },
            unbind:function(a,c){
                if(!!this._handlers[a])if(c){
...
provide("util/uri",function(a){
    using("util/querystring","util/util",function(b,c){
        ...
        for(var b=0,c;c=a[b];b++)if(c.getAttribute("rel")=="canonical")return d(c.getAttribute("href"));return null
        }
        function d(a){
        var b;
        if(a.match(/^https?:\/\//))return a;
        b=location.host,location.port.length>0&&(b+=":"+location.port);
        return[location.protocol,"//",b,a].join("")
        }
        a({
        absolutize:d,
        getCanonicalURL:e,
...
provide("tfw/widget/tweet",function(a){
    using("util/util","tfw/widget/base","util/querystring","util/uri",function(b,c,d,e){
        var f=document.title,g=encodeURI(location.href),h={
            en:1,
            de:1,
            es:1,
            fr:1,
            ...
            var b=this.params(),c=b.count||a.getAttribute("data-count"),d=b.lang||a.getAttribute("data-lang");
            this.text=b.text||a.getAttribute("data-text")||f,this.via=b.via||a.getAttribute("data-via")||e.getScreenNameFromPage(),this.url=b.url||a.getAttribute("data-url")||e.getCanonicalURL()||g,this.related=b.related||a.getAttribute("data-related"),this.counturl=b.counturl||a.getAttribute("data-counturl"),this.searchlink=b.searchlink||a.getAttribute("data-searchlink"),this.placeid=b.placeid||a.getAttribute("data-placeid"),this.dnt=b.dnt||a.getAttribute("data-dnt")||"",this.supportCount[c]||(c="horizontal"),this.count=c,h[d]||(d="en"),this.lang=d
            };
            
        i.prototype=new c.Base,b.aug(i.prototype,{
            supportCount:{
                vertical:1,
                horizontal:1,
                none:1
            ...
            titles:{
                ...
                id:"Twitter untuk Situs Web: Tombol Tweet",
                it:"Tweeter per i siti web: Bottone Tweet",
                ja:"WEBã‚µã‚¤ãƒˆå‘ã‘Twitter: ãƒ„ã‚¤ãƒ¼ãƒˆãƒœã‚¿ãƒ³",
                ko:"Twitter ì›¹ë²„ì „: íŠ¸ìœ— ë²„íŠ¼",
                nl:"Twitter voor websites: tweetknop",
                pt:"Twitter para websites: BotÃ£o de Tweet",
                ru:"Ð¢Ð²Ð¸Ñ‚Ñ‚ÐµÑ€ Ð´Ð»Ñ Ð²ÐµÐ±-ÑÐ°Ð¹Ñ‚Ð°: ÐºÐ½Ð¾Ð¿ÐºÐ° Â«Ð¢Ð²Ð¸Ñ‚Ð½ÑƒÑ‚ÑŒÂ»",
                tr:"Web siteleri iÃ§in Twitter: Tweetle Butonu"
            },
            dimensionsMap:{
                en:{
                    vertical:[55,62],
        ...
        parameters:function(){
            ...
            a._=(new Date).getTime(),b.compact(a);
            return d.encode(a)
            },
        render:function(a){
            var b=a.assetUrl()+"/widgets/tweet_button.html#"+this.parameters(),c=this.dimensionsMap[this.lang][this.count],d=this.titles[this.lang];
            this.element=this.create(b,"twitter-share-button twitter-count-"+this.count,c,d)
            }
        }),a({
        Embeddable:i
    })
...
provide("tfw/widget/follow",function(a){
    using("util/util","tfw/widget/base","util/querystring","util/uri",function(b,c,d,e){
        var f={
            ...
            this.originElement=a,this.id=this.generateId();
            var b=this.params(),c=b.lang||a.getAttribute("data-lang")||"en";
            this.showScreenName=b.show_screen_name||a.getAttribute("data-show-screen-name")||"",this.showCount=b.show_count||a.getAttribute("data-show-count")||"",this.button=b.button||a.getAttribute("data-button")||"blue",this.linkColor=b.link_color||a.getAttribute("data-link-color")||"",this.textColor=b.text_color||a.getAttribute("data-text-color")||"",this.width=b.width||a.getAttribute("data-width")||"",this.screenName=this.screenNameFromHref(),this.align=b.align||a.getAttribute("data-align")||"",this.lang=c,this.dnt=b.dnt||a.getAttribute("data-dnt")||"",f[c]||(this.lang="en")
            };
            
        g.prototype=new c.Base,b.aug(g.prototype,{
            titles:{
                en:"Twitter For Websites: Tweet Button",
                de:"Twitter fÃ¼r Webseiten: Tweet-Folgen",
                es:"Twi`tter para sitios web: BotÃ³n para Seguir",
                fr:'Twitter pour votre site web : bouton "Suivre"',
                id:"",
                it:"Tweeter per i siti web: Bottone Seguire",
                ja:"WEBã‚µã‚¤ãƒˆå‘ã‘Twitter",
                ko:"Twitter ì›¹ë²„ì „",
                pt:"Twitter para websites",
                ru:"Ð¢Ð²Ð¸Ñ‚Ñ‚ÐµÑ€ Ð´Ð»Ñ Ð²ÐµÐ±-ÑÐ°Ð¹Ñ‚Ð°",
                tr:"Web siteleri iÃ§in Twitter"
            },
            langs:{
                en:{
                    dimensions:[300,20]
        ...
        render:function(a){
            if(!!this.screenName){
                var b=a.assetUrl()+"/widgets/follow_button.html#"+this.parameters(),c=this.langs[this.lang].dimensions;
...
window.twttr=window.twttr||{},function(){
    ...
    twttr.events={
        ...
        },using("util/util","tfw/widget/follow","tfw/widget/tweet","tfw/widget/intent","util/events","tfw/widget/base",function(b,c,d,e,f,g){
    function k(){
        using("xd/parent","xd/jsonrpc",function(a,b){
            var c=h.assetUrl()+"/widgets/hub.html";
...
h.widgets={
    ...
    body:e.Listener
    },h.assetUrl=function(a){
    return j(a)+"://"+twttr.host
    },twttr.events=b.aug({},f.Emitter),twttr.events.oldbind=twttr.events.bind,twttr.events.bind=function(a,b){
    k(),this.bind=this.oldbind,this.bind(a,b)
    };
    
for(var l=0;handler=a[l];l++)twttr.events.bind(handler[0],handler[1]);
twttr.widgets={
    load:function(){
        g.init(h),g.embed()
        }
